...
try {
	...
	$aExtraParams = ['open' => $bDoSearch, 'action' => utils::GetAbsoluteUrlAppRoot().'pages/exec.php', 'exec_module' => 'itop-synchro-dashboard', 'exec_page' => 'dashboard.php'];
	...
	foreach ($aFilterCodes as $sFilterCode) {
		...
		if ($bDoSearch && $externalFilterValue != "") {
			...
			} elseif (count($externalFilterValue) == 1) {
	...
	while ($oSDS = $oSDSSet->Fetch()) {
		...
		$oSetSynchroLog = new CMDBObjectSet(DBObjectSearch::FromOQL($sSelectSynchroLog), ['start_date' => false] /* order by*/, ['source_id' => $oSDS->GetKey()]);
		$oSetSynchroLog->SetLimit(1); // Stats are based on the latest run
		$iDSid = $oSDS->GetKey();

		$sReplicaURl = utils::GetAbsoluteUrlAppRoot().'synchro/replica.php';

		$aRow = ['name' => $oSDS->GetHyperlink()];
	...
	$aConfig = [
		'name' => ['label' => MetaModel::GetName('SynchroDataSource'), 'description' => MetaModel::GetClassDescription('SynchroDataSource')],
		'start_date' => ['label' => Dict::S('UI:SynchroDashboard:StartDate'), 'description' => Dict::S('UI:SynchroDashboard:StartDate+')],
		'duration' => ['label' => Dict::S('UI:SynchroDashboard:Duration'), 'description' => Dict::S('UI:SynchroDashboard:Duration+')],
		'replicas' => ['label' => Dict::S('UI:SynchroDashboard:NbOfReplicas'), 'description' => Dict::S('UI:SynchroDashboard:NbOfReplicas+')],
		'errors' => ['label' => Dict::S('UI:SynchroDashboard:NbOfErrors'), 'description' => Dict::S('UI:SynchroDashboard:NbOfErrors+')],
		'warnings' => ['label' => Dict::S('UI:SynchroDashboard:NbOfWarnings'), 'description' => Dict::S('UI:SynchroDashboard:NbOfWarnings+')],
		'peak_memory' => ['label' => Dict::S('UI:SynchroDashboard:PeakMemoryUsage'), 'description' => Dict::S('UI:SynchroDashboard:PeakMemoryUsage+')],
	];
